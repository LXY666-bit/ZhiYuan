...
from ..core.auth import authenticate_user, create_access_token, get_current_user, get_db, get_password_hash, require_admin, resolve_role
from ..rag.document_loader import DocumentLoader
from ..vectorstore.embedding import embedding_service
from ..vectorstore.milvus_client import MilvusManager
from ..vectorstore.milvus_writer import MilvusWriter
from ..core.models import User
from ..vectorstore.parent_chunk_store import ParentChunkStore
...
@router.post("/documents/upload/jobs/{job_id}/cancel", response_model=DocumentUploadJobResponse)
async def cancel_upload_job(job_id: str, _: User = Depends(require_admin)):
    """取消正在进行的上传任务。"""
    job = upload_job_manager.cancel_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="上传任务不存在或已过期")
    return DocumentUploadJobResponse(**job)
...
async def delete_user(
    ...
    db.delete(target)
    db.commit()
    return {"message": f"成功删除用户 {username}"}
...
async def update_user_role(
    ...
    if new_role not in ("user", "admin"):
        raise HTTPException(status_code=400, detail="无效的角色，必须是 user 或 admin")
    ...
    if target.role == new_role:
        return {"message": f"用户 {username} 已经是 {new_role}", "role": target.role}

    target.role = new_role
    db.commit()
    return {"message": f"用户 {username} 角色已更新为 {new_role}", "role": target.role}
