...
def _set_last_web_context(context: dict):
    global _LAST_WEB_CONTEXT
    _LAST_WEB_CONTEXT = context


def get_last_web_context(clear: bool = True) -> Optional[dict]:
    """获取最近一次联网搜索上下文，默认读取后清空。"""
    global _LAST_WEB_CONTEXT
    context = _LAST_WEB_CONTEXT
    if clear:
        _LAST_WEB_CONTEXT = None
    return context
...
def search_knowledge_base(query: str) -> str:
    ...
    print(f"[DEBUG] RAG docs count: {len(docs)}")
...
def search_web(query: str) -> str:
    """Search the internet for real-time, up-to-date information. Use this for questions about current events, recent news, or anything that requires the latest information not found in the local knowledge base."""
    ...
    try:
        ...
        if not results:
            emit_rag_step("⚠️", "联网搜索未找到结果", "")
            return "No relevant results found on the web."

        emit_rag_step("✅", f"联网搜索完成，找到 {len(results)} 条结果", "")
        ...
        for i, r in enumerate(results, 1):
            ...
            sources.append({"title": title, "url": url, "content": content[:300]})
            formatted.append(f"[{i}] {title}\nURL: {url}\n{content}")
        _set_last_web_context({"query": query, "sources": sources})
        return "Web Search Results:\n" + "\n\n---\n\n".join(formatted)
    except ImportError:
        emit_rag_step("⚠️", "Tavily 库未安装", "")
        return "Tavily search library is not installed. Run: uv sync"
    except Exception as e:
        emit_rag_step("⚠️", f"联网搜索失败: {e}", "")
